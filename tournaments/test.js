...
50 k -> 1k + 4k + 3k + 4k = 12k

float taxCalculator()

*/

float taxCalculator(List<TaxBracket> tax, int income) {
  float result = 0;
  int currIdx = 0;
  while (0 < income) {
    if (-1 != tax[currInd].treshhold || tax[currInd].treshhold <= income) {
      income -= tax[currInd].treshhold;
      result += tax[currInd].treshhold * tax[currInd].rate;
    } else {
      result += income * tax[currInd].rate;
      income = 0;
    }
    ++currInd;
  }
...
Node LCA(Node x, Node y) {
  ...
  if (null == x.parent) return x;
  if (null == y.parent) return y;
