/*
 * Basic idea:
 * - O(nm) algorithm too slow.
 * - create rolling hash for both string:
 *       h[n] = s[n] ^ (s[n-1] << 1) ^ ... ^ (s[n-31] << 31)
 *   it's unattached to the string position,
 *   the number of faulty hits is high,
 *   but better filter than O(nm) algorithm.
 * - using binary search to find largest substring
 *
 * variables:
 * sh: s hash values for each position
 * sl: s hash position list, indexed by hash values. it's for >31 length substrings.
 * th: t hash values for each position
 * ha: hash value
 * l, m, r: position indexes for binary search. left, middle, right
 * found: flag for substring found
 * i, j: for loops
 */
